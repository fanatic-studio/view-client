...
export default class BasicLayout extends Vue {
	...
	async created() {
		...
		await this.getProjectLis(comonParams);
		const pjmParams = {
			pageIndex: 1,
			pageSize: 100,
			status: '',
	...
	listenRouteChange(
		...
		if (
			this.routesInfoMap[newpath.name] &&
			!this.routesInfoMap[newpath.name].public
		) {
			this.handleTab({
				id: newpath.name,
	...
	render() {
		...
		const {
			...
			fixedHeader,
			globalScroll,
			siderMenu: { collapsed, open },
			fixedLeftMenu,
		} = this;
		return (
			<section
				class={`${styles.basicLayout} ${
							...
							style={
								fixedHeader && globalScroll
									? {
											...
											width: `calc(100% - ${
												collapsed ? (deviceType === 'mobile' ? 0 : 80) : 256
											}px)`,
									  }
									: { background: '#fff!important' }
							}
						...
						>
							...
							<div class={styles.navTop}>
								<SiderMenu
									...
									menu={menuTree}
									tabActive={tabActive}
									class={`${styles.siderMenu} ${styles.siderMenuTop} ${
										theme === 'dark' ? styles.siderMenuTopDark : ''
									}`}
									openSiderSubMenu={this.openSiderSubMenu}
								/>
							</div>
							<RightBox top theme={theme} deviceType={deviceType} />
						</header>
					)}
					<main
						class={`
            ${
							navLayout === 'top' && deviceType === 'desktop'
								? styles.contentTopNav
								: ''
						}
          ...
          `}
					>
						{navLayout === 'top' && deviceType === 'desktop' ? (
							...
							<TabManager menuCollapsed={collapsed} deviceType={deviceType} />
						) : null}
