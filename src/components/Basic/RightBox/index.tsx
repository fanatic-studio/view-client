...
export default class RightBox extends Vue {
	...
	async startLogout() {
		...
		if (!res)
			this.$message.error(
				this.$locale[this.language.current].login.logoutError
			);
		// 清除tab数组
		this.clearTab({
			vm: this,
	...
	render() {
		const { top, theme, language, accountInfo, deviceType } = this;
		return (
			<div
				class={`${styles.rightBox} ${
					deviceType !== 'desktop' ? styles.rightBoxMobile : ''
				} ${top ? styles[theme] : ''}`}
			>
				<Dropdown>
					<span class={`${styles.avaterWrap} system-theme-color`}>
						...
						<span>{accountInfo.accountData.nickName}</span>
					</span>
					<Menu slot="overlay">
