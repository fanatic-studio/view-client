...
const actions: ActionTree<ApplicationState, any> = {
	...
	async addApplicationWelcome(store, p: ApplicationWelcomeMode) {
		const params: AddApplicationWelcomeParams = {
			...
			title: p.title,
			desc: p.desc,
			welcomeType: p.welcomeType,
			welcomeImage: p.welcomeImage,
			welcomeJump: p.welcomeJump,
			welcomeSkip: p.welcomeSkip,
			welcomeWait: p.welcomeWait,
			welcomeLimitE: p.welcomeLimitE,
			welcomeLimitS: p.welcomeLimitS,
		};
		try {
