...
const colorInfo: { key: string; color: string } | undefined = themeColor.find(
	(item) => item.color === defaultThemeColor
);

Vue.use(Vuex);

// 生成语言列表
const createLanguagelist = (locale: {
	[propname: string]: { name: string };
...
export default new Vuex.Store({
	...
	mutations: {
		...
		RESET_LOCAL_STORE(state: { [propsName: string]: any }) {
			const initState: { [propsName: string]: any } = createState(
				defaultLanguage
			);
			// 只遍历已经缓存的key
