		...
		return (
			<div class={styles.example}>
				<div class={styles.section}>
					...
					<Alert
						type="info"
						message="可在系统配置中配置系统组件状态是否需要持久化（默认为打开），逻辑时借由vuex-persistedstate实现，打开配置后系统会在每次修改对应的值之际将最新的相关数据缓存到本地，在浏览器刷新后还原用户之前的配置"
					/>
					<h4>需要缓存的系统状态Key</h4>
					<Alert
						showIcon={false}
						banner
						message="下示所有的key都储存在vuex的根级状态中，路径为src/store/index.tsx。只有出现在下述数组中的key才会被缓存至本地。"
					/>
					<pre>
						<code ref="systemcode">
							{`
        ...
        return localStore
          ...
          `}
					...
					<Alert
						showIcon={false}
						banner
						message="系统会在两种情况下清空系统缓存：1、禁用系统数据持久化时。2、手动点击系统配置中底部的清除数据缓存按钮。"
					/>
					<pre>
