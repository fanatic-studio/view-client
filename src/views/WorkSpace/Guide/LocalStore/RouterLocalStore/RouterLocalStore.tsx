		...
		return (
			<div class={styles.example}>
				<div class={styles.section}>
					...
					<Alert
						type="info"
						message="系统支持将一些路由页面的数据缓存到本地，此类型缓存借助localforage完成，以支撑浏览器F5刷新后重现某些页面的数据。注：该功能系统默认关闭。"
					/>
					<h4>路由数据持久化测试</h4>
					<div style={{ margin: '12px' }}>
						当前系统是否已开启路由数据持久化：
						{this.routerLocalStore ? (
							<b style={{ color: 'green' }}>已开启</b>
						) : (
							<b style={{ color: 'red' }}>未开启</b>
						)}
					</div>
					<div style={{ margin: '12px' }}>
					...
					<div style={{ margin: '12px' }}>
						<Alert
							showIcon={false}
							banner
							message="点击按钮可使得state的值增长1，若处于路由数据持久化的状态下，刷新浏览器，此状态会被还原至刷新前的值。"
						/>
						<Button
							style={{ margin: '12px 0' }}
							type="primary"
						...
						<Alert
							showIcon={false}
							type="error"
							message="注：关闭Tab，如果当前路由的配置中指定了module字段，则系统将会清空掉该路由的缓存以及vuex中当前内存中的数据。手动点击系统设置底部清除数据缓存按钮，该本地缓存同样会清空。"
						/>
					</div>
					...
					<ul>
						<li>
							...
							<Alert
								type="info"
								message="下述例子展示如何创建当前路由的数据模块，关键步骤为：1、引入extendsLocalStore。2、引入clearStore。3、绑定至当前模块的mutations中。"
							/>
        ...
        if (mutation.type.indexOf('/') > -1) {
          ...
          `}
					...
					<Alert
						showIcon={false}
						banner
						message="在开启路由数据持久化时，系统初始化之际会读取本地的数据库长度，检测到存在本地缓存之际会执行还原操作。"
					/>
					<pre>
